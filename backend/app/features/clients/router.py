...
async def create_client(body: CreateClientRequest, auth: OrgAdmin, db: DB):
...
async def update_client(client_id: str, body: UpdateClientRequest, auth: OrgAdmin, db: DB):
    return await service.update_client(client_id, auth.organization_id, body.model_dump(exclude_none=True), db)


@router.delete("/{client_id}", status_code=204)
async def delete_client(client_id: str, auth: OrgAdmin, db: DB):
    await service.delete_client(client_id, auth.organization_id, db)


@router.post("/{client_id}/job-draft")
async def generate_job_draft(client_id: str, body: GenerateJobDraftRequest, auth: OrgAdmin, db: DB):
