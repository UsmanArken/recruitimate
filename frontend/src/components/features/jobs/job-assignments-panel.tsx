...
export function JobAssignmentsPanel({
  ...
  const load = useCallback(async () => {
    try {
      const data = await apiFetch<{ job: { assignments: Assignment[] }; members: Member[]; canManage: boolean }>(
  ...
  async function handleAssign(e: React.FormEvent) {
    ...
    try {
      await apiFetch(`/api/jobs/${jobId}/assignments`, {
        method: "POST",
  ...
  async function handleRemove(assignmentId: string) {
    setError(null);
    try {
      await apiFetch(`/api/jobs/${jobId}/assignments/${assignmentId}`, { method: "DELETE" });
      await load();
      router.refresh();
    } catch (e) {
