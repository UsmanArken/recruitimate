...
export function InterviewQuestionBankPanel({
  ...
  async function generate() {
    ...
    setError(null);
    try {
      const data = await apiFetch<InterviewQuestionBankResult>(
        `/api/jobs/${jobId}/interview-questions`,
        { method: "POST", body: JSON.stringify({ focus, count: compact ? 8 : 12 }) }
      );
      setBank(data);
    } catch (e) {
      setError(e instanceof ApiError ? e.message : "Could not generate questions");
    } finally {
      setLoading(false);
    }
  }
