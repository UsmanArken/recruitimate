...
export function LiveInterviewAssistPanel({
  ...
  const fetchSuggestions = useCallback(
    async (text: string) => {
      ...
      setError(null);
      try {
        const data = await apiFetch<LiveAssistResult>(
          `/api/applications/${applicationId}/live-assist`,
          {
            method: "POST",
            body: JSON.stringify({ transcript: text, interviewId }),
          }
        );
        setAssist(data);
        lastFetchedLengthRef.current = text.length;
      } catch (e) {
        setError(e instanceof ApiError ? e.message : "Could not load suggestions");
      } finally {
        setLoading(false);
      }
    },
